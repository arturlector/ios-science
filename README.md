...
- Объявление `свойств (property): retain, assign, nonatomic, readonly, copy, weak, strong, unsafe_unretained`? 
...
- Вопрос о циклах в графах владения, и почему свойства delegate (предоставляющие доступ к делегату) обычно задаются как `assign`в MRC или `weak в ARC`?
...
- Что такое `retain cycle`? Приведите пример использо
...
- В чем отличия `колбэков (callbacks)`: `selectors` vs `delegate protocols` vs `blocks`?
...
- Что такое `View` (представление) и что такое `window` Цикл жизни `UIView`??
...
- Как работает `UITableView`? Цикл жизни UITableView?
